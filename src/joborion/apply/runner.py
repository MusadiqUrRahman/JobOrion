...
from joborion.apply import prompt as prompt_mod
from joborion.apply.browser import (
    launch_chrome, cleanup_worker, kill_all_chrome,
    reset_worker_dir, cleanup_on_exit, _kill_process_tree,
    BASE_CDP_PORT,
)
from joborion.apply.live_display import (
...
def _load_blocked():
    from joborion.config import load_blocked_sites
...
_active_agent_procs: dict[int, subprocess.Popen] = {}
_agent_proc_lock = threading.Lock()
...
def _make_mcp_config(cdp_port: int) -> dict:
    """Build MCP config dict for a specific CDP port (Claude Code format)."""
    return {
        "mcpServers": {
            "playwright": {
                "command": "npx",
...
def _make_opencode_mcp_config(cdp_port: int) -> dict:
    """Build MCP config dict for OpenCode format."""
    return {
        "mcp": {
            "playwright": {
                "type": "local",
...
def acquire_job(target_url: str | None = None, min_score: int = 7,
    ...
    try:
        ...
        from joborion.config import is_manual_ats
...
def run_job(job: dict, port: int, worker_id: int = 0,
    ...
    """Spawn an OpenCode session for one job application.
    ...
    # Write per-worker MCP config (OpenCode format)
    mcp_config_path = config.APP_DIR / f".mcp-apply-{worker_id}.json"
    mcp_config_path.write_text(json.dumps(_make_opencode_mcp_config(port)), encoding="utf-8")

    # Build claude command (COMMENTED OUT - using OpenCode instead)
    ...
    # Map model names for OpenCode (needs provider/model format)
    model_map = {
        "haiku": "opencode/deepseek-v4-flash-free",
        "sonnet": "opencode/deepseek-v4-flash-free",
        "opus": "opencode/deepseek-v4-flash-free",
    }
    opencode_model = model_map.get(model, model)
    cmd = [
        opencode_bin,
        "run",
        "--format", "json",
    ...
    # Write opencode.json with Playwright + Gmail MCP to worker directory
    opencode_config = {
    ...
    (worker_dir / "opencode.json").write_text(json.dumps(opencode_config, indent=2), encoding="utf-8")

    cmd.append("Follow the instructions in the attached file exactly.")
    ...
    try:
        ...
        with _agent_proc_lock:
            _active_agent_procs[worker_id] = proc
                ...
                try:
                    ...
                    if msg_type == "text":
                        ...
                        if text:
                            ...
                            lf.write(text + "\n")
                    elif msg_type == "tool_use" or msg_type == "tool-call":
                        name = msg.get("part", {}).get("name", msg.get("name", ""))
                        inp = msg.get("part", {}).get("input", msg.get("input", {}))
                        if "url" in inp:
                    ...
                    # Claude Code JSON format (kept as fallback)
        ...
        with _agent_proc_lock:
            _active_agent_procs.pop(worker_id, None)
...
def main(limit: int = 1, target_url: str | None = None,
    ...
    def _sigint_handler(sig, frame):
        ...
        if _ctrl_c_count == 1:
            ...
            with _agent_proc_lock:
                for wid, cproc in list(_active_agent_procs.items()):
