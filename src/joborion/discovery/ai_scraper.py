...
from joborion.config import CONFIG_DIR
from joborion.database import init_db, get_stats
...
def _store_jobs_filtered(
    ...
    for job in jobs:
        ...
        if not url:
            continue
        if not _location_ok(job.get("location"), accept_locs, reject_locs, remote_only):
            filtered += 1
            continue
        try:
            conn.execute(
    ...
    def _process_result(r: dict, target: dict) -> None:
        ...
        if jobs:
            new, existing = _store_jobs_filtered(conn, jobs, target["name"],
                                                  r.get("strategy", "?"),
                                                  accept_locs, reject_locs, remote_only)
            total_new += new
...
def scrape_ai_sites(
    ...
    return _run_all(targets, accept_locs, reject_locs, workers=workers, remote_only=remote_only)
