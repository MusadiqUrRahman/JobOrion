...
parses salary ranges, and stores everything in the JobOrion database.
...
from joborion.database import get_connection, init_db
        ...
        try:
            conn.execute(
                ...
                (url, title, company, salary, description, location_str, site_label, strategy, now,
                 full_description, apply_url, detail_scraped_at, "jobspy"),
            )
            new += 1
...
def scrape_jobspy(cfg: dict | None = None, mode: str | None = None) -> dict:
    ...
    if not cfg:
        log.warning("No search configuration found. Run `joborion init` to create one.")
